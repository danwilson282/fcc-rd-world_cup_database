...
echo "$($PSQL "SELECT name FROM games INNER JOIN teams ON (games.opponent_id=teams.team_id OR games.winner_id=teams.team_id) WHERE round='Eighth-Final' AND year=2014 ORDER BY name ASC")"
